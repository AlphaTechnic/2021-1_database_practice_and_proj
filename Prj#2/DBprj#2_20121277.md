...
where BORROWS.studentid=STUDENTS.studentid
and STUDENTS.class='C1';
```
...
```sql
select name, count(studentid) as num_of_read 
from (select * from BORROWS) 
natural join (select * from STUDENTS where students.class='C1') 
group by name;
```
